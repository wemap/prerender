...
const TABS_COUNT = process.env.TABS_COUNT || 1;
...
server.init = function(options) {
	...
	this.options.tabsCount = this.options.tabsCount || TABS_COUNT;

	this.browser = require('./browsers/chrome');

	// initialize tabs
	this.availableTabs = [];
	this.waitForTabQueue = [];

	return this;
};

server.waitForTab = function(req) {
	const promise = new Promise((resolve) => {
		if (this.availableTabs.length) {
			console.log('A tab is available');
			const tab = this.availableTabs.pop();
			resolve(tab);
		} else {
			console.log('No tab available, will wait');
			this.waitForTabQueue.push(resolve);
		}
	})
	.then(tab => {
		console.log('Got a tab');
		if (tab.prerender && tab.prerender.url) {
			tab.previousUrl = tab.prerender.url;
		}
		tab.browser = this.browser;
		tab.prerender = Object.assign({}, tab.prerender, req.prerender);
...
server.releaseTab = function(tab) {
	console.log('Release tab');
	if (this.waitForTabQueue.length) {
		const resolve = this.waitForTabQueue.pop();
		resolve(tab);
	} else {
...
server.startPrerenderTabs = function() {
	console.log(`this.options.tabsCount = ${this.options.tabsCount}`);

	let openedTabs = 0;
	const iterate = () => {
		console.log(openedTabs);
		console.log(`tab ${openedTabs}/${this.options.tabsCount}`);
		if (openedTabs < this.options.tabsCount) {
			this.browser.openTab({})
				.then(tab => {
					this.availableTabs.push(tab);
					openedTabs++;
				...
				.catch(e => {
					console.error('error while opening tab');
					console.error(e);
				});
		}
	}
...
server.onRequest = function(req, res) {
	...
	.then(() => {
		...
		const newUrl = urlParse(req.prerender.url);
		const query = queryString.parse(newUrl.query.substr(1));
		const hash = query.wemapHash ? decodeURIComponent(query.wemapHash) : '';

		let url = `${newUrl.origin}${newUrl.pathname}${hash}`;

		if (req.prerender.tab.previousUrl) {
			const previousUrl = urlParse(req.prerender.tab.previousUrl);

			if (previousUrl.origin === newUrl.origin
				&& previousUrl.pathname === newUrl.pathname
				&& hash.length
			) {
				url = `${previousUrl.origin}${previousUrl.pathname}${hash}`;
			}
		}

		return this.browser.loadUrlThenWaitForPageLoadEvent(req.prerender.tab, url);
	}).then(() => {
...
server.finish = function(req, res) {
	...
	if (req.prerender.tab) {
		this.releaseTab(req.prerender.tab);
	}
