...
const TABS_COUNT = process.env.TABS_COUNT || 2;
...
server.releaseTab = function(tab) {
	if (this.waitForTabQueue.length) {
		const {prerender, resolve } = this.waitForTabQueue.pop();
